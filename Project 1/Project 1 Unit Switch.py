...
line = input("\nEnter the initial velocity (in ft/s): ")
Velocity = float(line)

line = input("\nEnter the angle of the Red Bird with respect to the horizontal axis (in degrees): ")
Angle = float(line)

line =  input("\nEnter the ground distance to the structure (in feet): ")
GroundDistance = float(line)

# Come back and change so user can adjust height of launch

print("\nFor right now, the Red bird will be launched off a 5 foot tall slingshot")

line = input("\nEnter the height of the structure your're launching the Red Bird at (in feet): ")
...
print("\nThe Red Bird was traveling at a velocity of",round(Vf, 5),"ft/s")
...
StructureHit = 5 + (Vy * Time) + (.5 * -9.8 * (math.pow(Time, 2)))

print("\nThe Red bird was at a height of",round(StructureHit, 5),"ft from the ground when it reached the intended structure")
